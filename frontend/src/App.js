...
function App() {
  ...
  const updateSettings = async () => {
    const res = await fetch('http://localhost:8000/update_settings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const updateBotStatus = async (status) => {
    const res = await fetch('http://localhost:8000/bot_status', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const fetchBalance = async () => {
    const res = await fetch('http://localhost:8000/binance_balance');
    if (!res.ok) {
      const data = await res.json();
